...
if not getconnections or not hookmetamethod or not getnamecallmethod or not ((getgenv and getgenv()) or _G) then
	game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Prisonhook Admin",Text = "Executor is not supported!",Duration = 10,})
end
if not workspace:FindFirstChild("Criminals Spawn") or not workspace:FindFirstChild("Criminals Spawn"):FindFirstChild("SpawnLocation") then
	game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Prisonhook Admin",Text = "Criminals spawn not found! Please rejoin.",Duration = 10,})
end
game:GetService("Players").LocalPlayer.Character:WaitForChild("HumanoidRootPart")
if game:FindFirstChild("Prisonhook_revamp_loaded") then  ((getgenv and getgenv()) or _G).NotifPrisonhook("Prisonhook admin is already executed!",false) return warn("Already loaded") end
...
--------
Folder.Name = "Prisonhook_revamp_loaded"
...
function API:CreateCmd(Header, Description, Callback,IsHide,Extra,IsPre,plsdonotlower)
	...
	local function FactCheck(TheText)
		...
		if First:lower() == Header:lower() or First:lower() == Prefix..Header:lower() then
			...
			if not a and b then
				warn("--> PRISONHOOK ADMIN DETECTED AN ERROR")
...
GlobalVar.NotifPrisonhook = function(t,v)
	API:Notif(t,v)
end
local States = {}
local Settings = {
...
do
	if PremiumActivated then
		...
		API:CreateCmd("anticrash", "Tries to stop simple crashes (DOESNT WORK WITH PRISONHOOK ADMIN CRASH)", function(args)
		...
		API:CreateCmd("autoadmin", "Admins someone even if you leave and rejoin", function(args)
			local Target = API:FindPlayer(args[2])
			if Target then
				local Content = game:GetService("HttpService"):JSONDecode(readfile("Prisonhook_Admin/SavedAdmins.json"))
				...
				if not Found then
					...
					local ohString2 = "All"
					game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(ohString1, ohString2)
					writefile("Prisonhook_Admin/SavedAdmins.json",game:GetService("HttpService"):JSONEncode(New))
	...
	--//end of premium
	API:CreateCmd("drag", "Enables draggable mode for all prisonhook guis", function(args)
	...
	API:CreateCmd("allcmds", "tells you the ammount of commands prisonhook admin has", function(args)
		API:Notif("Prisonhook admin has "..tostring(AmmountCurrent).." commands.")
	end)
	...
	API:CreateCmd("unload", "Destroys the script unloading it", function(args)
		API:Destroy(game:FindFirstChild("Prisonhook_revamp_loaded"))
		Unloaded = true
